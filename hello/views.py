...
def home(request):
    context = {
        ...
        'data_pts': json.dumps([str(d[0]) for d in DataPoint.objects.order_by('-gathered_at').values_list('value')[:30]]),
        'data_dates': json.dumps([d[0] for d in DataPoint.objects.order_by('-gathered_at').values_list('gathered_at')[:30]]),
    }
